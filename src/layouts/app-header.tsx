...
export default function AppHeader({ 
  ...
  return (
    <>
          ...
          <aside className="fixed top-0 right-0 bottom-0 w-64 bg-white shadow-2xl z-50 sm:hidden transform transition-transform duration-300 ease-in-out">
            <div className="flex flex-col h-full">
              ...
              <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
                ...
                {/* Mobile: Account / Login section - always expanded */}
                <div className="pt-3 mt-3 border-t border-gray-200">
                  <p className="px-4 py-2 text-xs font-semibold text-gray-500 uppercase tracking-wider">
                    {isNepali ? 'अकाउन्ट' : 'Account'}
                  </p>
                  ...
                  {!authUser ? (
                    <div className="px-4">
                      <button
                        ...
                        className="w-full px-4 py-2.5 bg-rose-600 text-white text-sm font-medium rounded-lg hover:bg-rose-700 transition-colors"
                      >
                        {isNepali ? 'लगइन' : 'Login'}
                      </button>
                    </div>
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center gap-3 px-4 py-2.5 text-gray-600">
                        <img src={authUser?.photoURL || '/favicon/user.svg'} alt="avatar" className="h-8 w-8 rounded-full border border-gray-200" />
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-gray-900 truncate">{authUser?.displayName || 'User'}</p>
                      ...
                      {profile?.role === 'astrologer' && (
                        ...
                        >
                          ...
                          <span className="text-sm font-medium">{isNepali ? 'प्रोफाइल' : 'Profile'}</span>
                        </Link>
                      )}
                      ...
                      >
                        ...
                        <span className="text-sm font-medium">{isNepali ? 'अपोइन्टमेन्टहरू' : 'Appointments'}</span>
                      </Link>
                      ...
                      {profile?.role === 'super_admin' && (
                        ...
                        >
                          ...
                          <span className="text-sm font-medium">{isNepali ? 'प्रयोगकर्ताहरू' : 'Users'}</span>
                        </Link>
                      )}
                      ...
                      <button
                        ...
                        className="w-full flex items-center gap-3 px-4 py-2.5 text-gray-700 hover:bg-amber-50 hover:text-rose-700 rounded-lg transition-colors"
                      >
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="h-5 w-5">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15M12 9l-3 3m0 0l3 3m-3-3h12.75" />
                        </svg>
                        <span className="text-sm font-medium">{isNepali ? 'साइन आउट' : 'Sign out'}</span>
                      </button>
                    </div>
                  )}
                </div>
      ...
      {/* Mobile Login Dialog */}
      <LoginDialog open={showMobileLoginDialog} onClose={() => setShowMobileLoginDialog(false)} />
    </>
  );
}
