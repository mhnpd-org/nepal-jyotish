...
const VimAntardashaTable: React.FC<{ data: NonNullable<VimshottariDasha['antardashas']> }> = ({ data }) => {
        ...
        <table className="w-full text-xs sm:text-sm border-collapse">
          <thead className="bg-gradient-to-r from-red-50 via-amber-50 to-red-50">
            <tr className="text-red-700">
              ...
              <th className="border border-red-200 px-2 py-1 font-semibold">अन्तर्दशा</th>
              <th className="border border-red-200 px-2 py-1 font-semibold whitespace-nowrap">अवधि (BS)</th>
              <th className="border border-red-200 px-2 py-1 font-semibold">वर्ष</th>
              <th className="border border-red-200 px-2 py-1 font-semibold">मास</th>
              <th className="border border-red-200 px-2 py-1 font-semibold">दिन</th>
              <th className="border border-red-200 px-2 py-1 font-semibold">योग</th>
            </tr>
          </thead>
          <tbody>
            {data.map((ad, i) => {
  ...
  <span className="inline-block h-2 w-2 rounded-full bg-amber-300" /> वर्तमान अन्तर्दशा
      </div>
    </div>
  );
};
      ...
      <div className="flex flex-col gap-1 text-[15px] sm:text-base leading-relaxed">
        <div className="flex flex-wrap items-baseline gap-2">
          <h3 className="text-base sm:text-lg font-semibold text-red-700 tracking-wide">
            {translateSanskritSafe(item.dashaLord)} महादशा
          </h3>
...
export default function VimshottariDashaVerticalPage() {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.error(e);
        if (mounted) {
          setError("विंशोत्तरी दशा लोड गर्न सकिएन। कृपया जन्म विवरण पुनः जाँच गर्नुहोस्।");
  ...
  if (!kundali) return <LoadingState message="विंशोत्तरी दशा तयार गरिँदै..." />;
    ...
    <div className="space-y-4">
      <header className="space-y-1">
        ...
        <p className="text-sm text-gray-600 leading-relaxed">
          विंशोत्तरी महादशा क्रम र यसको अन्तर्दशा विवरण
        </p>
      </header>
      ...
      <p className="text-[11px] sm:text-xs text-gray-500 italic pt-1">
        वर्तमान अन्तर्दशा हल्का रङ्गमा.
      </p>
    </div>
  );
}
