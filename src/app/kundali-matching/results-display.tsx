...
export default function MatchingResultsDisplay({
  ...
  return (
    <main className="min-h-screen w-full bg-gradient-to-br from-rose-50 via-orange-50 to-amber-50">
      <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="bg-white rounded-lg shadow p-4 sm:p-6 mb-6">
                ...
                <div key={key} className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg p-3 border border-gray-200">
                  ...
                  <div className="text-xs mt-1">
                    <span className={`inline-block px-2 py-1 rounded ${
                      value.verdict === "Good" ? "bg-green-100 text-green-800" :
                      value.verdict === "Average" ? "bg-yellow-100 text-yellow-800" :
          ...
          <div className="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
            <p className="text-xs text-gray-700">
              ...
              {(dosha.nadiDosha.present || dosha.bhakootDosha.present) && (
                <span className="text-orange-700"> • {dosha.nadiDosha.present && "नाडी"}{dosha.nadiDosha.present && dosha.bhakootDosha.present && "/"}{dosha.bhakootDosha.present && "भकूट"} दोष</span>
              )}
            </p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow p-4 sm:p-6 mb-6">
          ...
          <div className="space-y-4">
            ...
            <div className="border-l-4 border-rose-500 pl-3">
              <h3 className="font-bold text-base mb-2 text-gray-800">मांगलिक दोष <span className="text-xs font-normal">(Manglik Dosha)</span></h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs sm:text-sm">
                <div>
                  <p className="font-semibold text-blue-900">वर:</p>
                  <p>{dosha.manglikDosha.male.isManglik ? `मांगलिक (${dosha.manglikDosha.male.strength})` : "मांगलिक होइन"}</p>
                </div>
                <div>
                  <p className="font-semibold text-pink-900">वधु:</p>
                  <p>{dosha.manglikDosha.female.isManglik ? `मांगलिक (${dosha.manglikDosha.female.strength})` : "मांगलिक होइन"}</p>
                </div>
              </div>
              <div className="mt-2 p-2 bg-gray-50 rounded">
                <p className="text-xs">
                  <span className={`font-semibold ${dosha.manglikDosha.compatible ? "text-green-600" : "text-red-600"}`}>
            ...
            <div className="border-l-4 border-purple-500 pl-3">
              <h3 className="font-bold text-base mb-2 text-gray-800">नाडी दोष <span className="text-xs font-normal">(Nadi Dosha)</span></h3>
              <p className={`text-sm font-semibold ${dosha.nadiDosha.present ? "text-red-600" : "text-green-600"}`}>
              ...
              {dosha.nadiDosha.present && (
                <div className="mt-2 text-xs sm:text-sm space-y-1">
                  ...
                  {dosha.nadiDosha.cancellationPossible && <p className="text-green-600">निवारण सम्भव (Remedies possible)</p>}
            ...
            <div className="border-l-4 border-amber-500 pl-3">
              <h3 className="font-bold text-base mb-2 text-gray-800">भकूट दोष <span className="text-xs font-normal">(Bhakoot Dosha)</span></h3>
              <p className={`text-sm font-semibold ${dosha.bhakootDosha.present ? "text-red-600" : "text-green-600"}`}>
              ...
              {dosha.bhakootDosha.present && (
                <div className="mt-2 text-xs sm:text-sm space-y-1">
                  ...
                  {dosha.bhakootDosha.cancellationPossible && <p className="text-green-600">निवारण सम्भव (Remedies possible)</p>}
            ...
            <div className="mt-3 p-3 rounded-lg bg-gradient-to-r from-gray-50 to-gray-100">
              <p className="font-bold text-sm">
                समग्र दोष स्थिति (Overall Dosha Status):
                <span className={`ml-2 ${
                  dosha.overallDoshaStatus === "Clear" ? "text-green-600" :
                  dosha.overallDoshaStatus === "Manageable" ? "text-yellow-600" :
                  "text-red-600"
                }`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          ...
          <div className="bg-white rounded-lg shadow p-4">
            ...
            <div className="space-y-2 text-xs sm:text-sm">
              <p className={`font-semibold ${rashiCompatibility.compatible ? "text-green-600" : "text-red-600"}`}>
                {rashiCompatibility.compatible ? "मिलेको" : "नमिलेको"}
              </p>
              <div>
                <p className="font-semibold text-gray-700">सम्बन्धको प्रकृति:</p>
                <p>{translateSanskritSafe(rashiCompatibility.relationshipNature)}</p>
              </div>
              <div>
                <p className="font-semibold text-gray-700">भावनात्मक बन्धन:</p>
          ...
          <div className="bg-white rounded-lg shadow p-4">
            ...
            <div className="space-y-2 text-xs sm:text-sm">
              <p className={`font-semibold ${nakshatraCompatibility.compatible ? "text-green-600" : "text-red-600"}`}>
                {nakshatraCompatibility.compatible ? "मिलेको" : "नमिलेको"}
              </p>
              <div>
                <p className="font-semibold text-gray-700">मानसिक तालमेल:</p>
                <p>{translateSanskritSafe(nakshatraCompatibility.mentalHarmony)}</p>
              </div>
              <div>
                <p className="font-semibold text-gray-700">शारीरिक तालमेल:</p>
                <p>{translateSanskritSafe(nakshatraCompatibility.physicalHarmony)}</p>
              </div>
              {nakshatraCompatibility.remarks && (
                <p className="text-xs text-gray-600 italic">{nakshatraCompatibility.remarks}</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            ...
            <ul className="space-y-1.5">
              {(overallAssessment.strengthsNp.length > 0 ? overallAssessment.strengthsNp : overallAssessment.strengths).map((strength: string, index: number) => (
                <li key={index} className="flex items-start gap-2 text-xs sm:text-sm">
                  ...
                  <span className="text-gray-700 line-clamp-2">{strength}</span>
                </li>
              ))}
            </ul>
          </div>
          ...
          {/* Concerns */}
          <div className="bg-gradient-to-br from-red-50 to-rose-50 rounded-lg shadow p-4 border border-red-200">
              ...
              <ul className="space-y-1.5">
                {(overallAssessment.concernsNp.length > 0 ? overallAssessment.concernsNp : overallAssessment.concerns).map((concern: string, index: number) => (
                  <li key={index} className="flex items-start gap-2 text-xs sm:text-sm">
                    ...
                    <span className="text-gray-700 line-clamp-2">{concern}</span>
                  </li>
                ))}
              </ul>
            ) : (
              <p className="text-sm text-gray-600">कुनै प्रमुख चिन्ता छैन</p>
            )}
          </div>
        </div>
